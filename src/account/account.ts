...
import { Asset as AssetApi } from '@findora-network/findora-sdk-v2-asset';
import {
  // getLedger,
  // FindoraWalletTypes as FindoraWallet,
  bigNumber, // AssetRules as LedgerAssetRules,
  // TransactionBuilder,
  // XfrKeyPair,
  // feeService,
...
import { Network, NetworkEnvironment, IssuedRecord, TxOutput } from '@findora-network/findora-sdk-v2-network';
// 2
// import { addUtxo, AddUtxoItem } from '../../services/utxoHelper';
import { AddUtxoItem, utxoHelper } from '@findora-network/findora-sdk-v2-utxo';
...
const { create: createBigNumber, fromWei } = bigNumber;
const { createKeypair, getAddressPublicAndKey } = KeypairApi;
const { addUtxo } = utxoHelper;
const { getAssetCode, getFraAssetCode } = AssetApi;
...
export const getAssetBalance = async (
  ...
  assetCode: string,
  sids: number[],
): Promise<bigNumber.BigNumberValue> => {
  let utxoDataList;
